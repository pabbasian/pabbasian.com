function Bit(size, color) {
    ...
    this.tick = function() {
        if (this.xpos > maxX) {
            ...
            this.xpos += this.speed * Math.PI;
        }
        this.ypos = 100 + 500 * (canvas.width - this.xpos) / canvas.width + 100 * Math.sin(this.offset + this.xpos / 500) * Math.PI;
    }

    this.draw = function() {
        context.fillStyle = this.color;
        context.fillRect(this.xpos, this.ypos, this.size, this.size);
        context.arc(this.xpos, this.ypos, this.size,0, this.size*Math.PI);
...
var bit;
for (var i = 0; i < 10; ++i) {
    bit = new Bit(10 + 400 * Math.random(), colours[Math.floor(4 * Math.random())]);
    bits[i] = bit;
}
